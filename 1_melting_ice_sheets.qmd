...
<https://github.com/MSCI-709-2026/unit_2_climate>

Fork the repository to your own GitHub profile. From there, clone the forked repository and open Positron. Go to `File -> New folder from Git...` and choose the folder on your hard drive where you are storing your files for this course. Now you have a copy of the unit_2_climate module on your local computer associated with the GitHub repo you forked to your profile. Now you have access to all of the scripts, data and documentation files in the unit_2 class repo. I would recommend creating new `.R` files in your local unit_2_climate directory to type in the commands that we go over in class as you follow along. This gives you practice typing out code, and gives you the comfort to try new things without being afraid that you'll "mess up" your copy of the tutorial. You can always refer back to the lesson tutorial that I wrote, which is sitting right in your local directory. As you follow along in your own script (or even the `.qmd` tutorial files) you can insert your code for the in-class exercises right in the middle of the scripts. When you make changes to the repository by adding new scripts or editing the existing scripts, don't forget to `Stage - Commit - Pull - Push` out your changes to your GitHub repo. Remember, those changes won't show up in the class repository, they'll only show up in **your** forked branch of the repository in your own GitHub profile.
...
![](doc/grace_satellites.jpg){width=75%}
...
Go to the website and register yourself as a NASA Earthdata user. From there you can click on the data and download it. If a pop-up doesn't show up when you click on the data, then in your web browser, go to `File -> Save page as -> and save it as a text file` in your local repo folder in a directory called `/data`.
...
plot(mass_Gt ~ decimal_date, 
     data=ant_ice_loss, 
     ylab="Antarctica Mass Loss (Gt)") #  ?plot.formula
