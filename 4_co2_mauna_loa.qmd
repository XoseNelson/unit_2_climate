...
So to grab the last 5 years of data, we only want values of `co2$decimal_date` greater than 2020. We can use a conditional statement `>` to signal values *greater than* 2020:

```{r}
# Showing 2 ways to subset
summary(co2$decimal_date > 2020) # vector of TRUES and FALSES
summary(which(co2$decimal_date > 2020)) # vector of indices that meet condition (see ?which for bells and whistles)
...
plot(seasonal_cycle ~ decimal_date, type="l", data=co2_2020to2025) 
```
